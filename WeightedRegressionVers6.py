...
def portion2(x, Matter):
    return 1/(math.sqrt((1+x)**2)*(1+Matter*x) - 2*(2+x)*math.fabs(1-Matter))

"""
This definition is the integration process, note the integration is from 0 to x and not 1 to x as per the equation E5.
"""
def intersum(x, Matter):
    return quad(portion2, 0, x, args=(Matter))[0]
